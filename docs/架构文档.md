...
> 目标：针对新话题的性能优化任务，本文档给出已知的性能瓶颈、优化方向、基准数据采集方法与验证门槛。

### 14.1 当前已知性能瓶颈

| 优先级 | 瓶颈 | 位置 | 现状 | 优化方向 |
|---|---|---|---|---|
| 🔴 高 | 每连接 schema 拉取未全量缓存 | SchemaCache 惰性拉取 + 内存缓存，但 renderer `fetchSchemaSnapshot` 每次连接切换全量拉表+前50张表字段 | 连接切换后首次补全/高亮有可见延迟 | ① 主进程 SchemaCache 已按 connectionId 缓存（ipc.ts）② renderer 侧缓存 snapshot（连接级）③ 懒加载字段：用户输入时才拉表结构 |
| 🟡 中 | 结果表格 5 万行虚拟滚动 | ResultGrid 固定行高 24px + overscan 10 | 滚动基本流畅，但排序/筛选是全量内存操作 | ① 排序/筛选 Web Worker 化 ② 列宽自定义缓存 ③ 行高自适应（monaco 虚拟列表模式） |
| 🟡 中 | 执行历史无分页 | HistoryPanel `history:list({limit:100})` 全量返回 | 100 条以内无感知，海量历史初始化慢 | IPC 分页（offset/limit）+ 虚拟滚动列表 |
| 🟡 中 | 查询执行是串行度量的 | QueryService 流式读取 + MAX_RESULT_ROWS 截断 | 大表 COUNT/聚合查询耗时集中在主进程 | ① 主进程异步执行不卡 UI（已天然满足）② 大结果集改为 `LIMIT` 流式 + 增量渲染 |
| 🟢 低 | 每次查询自动写历史 | ipc.ts query:execute handler 内 addHistory 同步写 SQLite | 高频查询有额外磁盘写 | ① 历史写入队列批处理 ② WAL 模式已启用，可评估 async 写 |
| 🟢 低 | Monaco 全量打包 | `loader.config({ monaco })` 本地加载后渲染 bundle 变大 | 135MB 安装包（含 monaco） | ① `vite-plugin-monaco-editor` 按需分包 ② Monaco 语言 worker 懒加载 |
...
# 单测基准
npx vitest run --reporter=verbose 2>&1 | grep -E "Duration|Tests"

# 冒烟基准（Windows GUI）：
# 1. 记录冷启动 → 窗口出现耗时
# 2. 连接 5 万行表，记录展开/滚动 FPS
# 3. 记录 schema 补全从触发到出建议的延迟
```

性能优化前后对比：对每个优化点记录「优化前基准值 / 优化后基准值」，验证门槛建议 `≥30% 提升` 或明确无感知项标注。

### 14.3 优化作业流程（新话题开工步骤）

1. 读本文档 §14.1 选择目标瓶颈（建议从 🔴 高 开始）
2. `git pull` 最新 master（含本会话全部修复）
3. 读目标代码模块（`lib/` 纯逻辑或 `components/`）
4. 写基准测试（vitest）或手动冒烟记录基准值
5. 实施优化 → 保持 199+ 测试全绿
6. 符合 R3 三门槛（tsc + vitest + build）后推送，CI 验证
7. 更新本文档 §14.1 状态 + 开发文档 §7 坑位

### 14.4 性能优化留意点（不要破坏的契约）

- **R5 类型唯一来源**：优化不能复制类型到进程内部
- **R6 渲染零直连**：性能优化不改变「一切经 IPC」架构
- **IPC 包装**：新增批量/分页通道时同步 `ipc-contract.ts` + preload 自动暴露 + 契约测试
- **虚拟滚动**：行高固定假设已用于 scrollTop 计算，改行高需同步改 `result-grid.tsx`
- **SchemaCache**：缓存键 `connectionId`，连接删除时清理（`connections:remove` 已先调 `closePool`，可顺手清 schema cache）