...
> **会话8 已完成一轮 7 项优化**（见 §14.1 完整状态）。下一轮可继续处理 🟡 中 ⏳ 待办项。

### 14.1 当前已知体验问题

| 优先级 | 问题 | 位置 | 现状 | 优化方向 |
|---|---|---|---|---|
| 🔴 高 | **字段注释未显示** | `ObjectExplorer.tsx` 展开字段列表只显示 `c.name` 和 `c.type`，`c.comment` 有数据但未展示 | ~~用户看不到字段的业务含义~~ ✅ 已修复（2026-08-20 会话8）：字段行追加 `.col-comment` 灰色小字 | 字段行追加注释文字（灰色小字），或 tooltip |
| 🔴 高 | **Excel 导出无反应** | `ExportMenu.tsx` 用 `window.prompt` 输入路径（Electron 中 prompt 返回 null → 导出静默失败） | ✅ 已修复（2026-08-20 会话8）：改用原生 `dialog:showSaveDialog`（新 IPC 通道）+ 导出目录持久化到 settings（`lastExportDir`） | ① 原生对话框 ② 路径持久化 ③ 导出成功 alert 反馈 |
| 🟡 中 | **预览按钮无功能** | `ObjectExplorer.tsx` 的「预览」按钮点击后只 `console.log` | ⏳ 待办 | 接入 `schema:dataPreview` IPC（已注册），弹窗或新标签显示前 N 行 |
| 🟡 中 | **查询取消按钮缺失** | `SqlEditor.tsx` 工具栏有「执行」无「停止」 | ⏳ 待办 | 在 `workspace` store 加执行状态，执行中显示「停止」按钮调 `query:cancel` |
| 🟡 中 | **打开/保存用 window.prompt** | `App.tsx` handleOpen/handleSave 用 `prompt()` 输入路径 | ⏳ 待办（`dialog:showSaveDialog` 已就绪，可复用） | 改为 Electron 原生文件对话框（`dialog.showOpenDialog`/`showSaveDialog`） |
| 🟡 中 | **连接状态未指示** | `ConnectionManager.tsx` 列表无连接状态指示 | ⏳ 待办 | 连接保存后自动测试，状态圆点（绿=已连/灰=未连/红=错误） |
| 🟢 低 | **双击表后标签未命名** | 现在用 `newTab()` 生成「未命名-N」 | ⏳ 待办（低优先级） | 用 `db.table` 或 `SELECT db.table` 作为标签标题 |
| 🟢 低 | **右键菜单缺失** | `ObjectExplorer.tsx` 无右键菜单 | ⏳ 待办 | 添加 `onContextMenu` 事件（刷新/DDL/数据预览） |
| 🆕 会话8 | **字段补全不支持表别名** | `sql-completion.ts` 只认精确表名 `xxx.`，`FROM users u WHERE u.` 不联想 | ✅ 已修复（2026-08-20 会话8）：新增 `extractTableAliases()` 解析 FROM/JOIN 段别名 | 输入别名. 时联想对应表字段 |
| 🆕 会话8 | **筛选行与表头错位 + 列宽不可调** | `ResultGrid.tsx` 表头/筛选/表体各自算宽（`COL_W` vs `COL_W-16`），无拖拽 | ✅ 已修复（2026-08-20 会话8）：三处共用单一 `gridTemplateColumns` + 表头 resize handle 拖拽列宽 | 单一列宽源根治错位 |
| 🆕 会话8 | **不选中语句执行报「找不到数据库」** | `SqlEditor.tsx` 默认库取 `databases[0]`（可能是 `information_schema`） | ✅ 已修复（2026-08-20 会话8）：默认库跳过系统库（`SYSTEM_DATABASES`）+ 执行时兜底（连接默认库 → 第一个用户库） | 跳过系统库 + 兜底链 |
| 🆕 会话8 | **标签切换/新建时编辑器卡顿（约 1-3s 无法输入）** | `App.tsx` 用 `key={activeTab.id}` 强制 Monaco 重挂载 | ✅ 已修复（2026-08-20 会话8）：去掉 key，改 `path={tab.id}` 单实例多 model + `main.tsx` 预热 `loader.init()` | 对齐 react-monaco-editor #269 官方多标签方案 |
| 🆕 会话8 | **双击字段不能插入编辑器** | `ObjectExplorer.tsx` 字段行无双击处理 | ✅ 已修复（2026-08-20 会话8）：`onInsertColumn` 回调 + SqlEditor `forwardRef` 暴露 `insertTextAtCursor` | 双击字段插入 `db`.`table`.`col` 到光标处 |
| 🆕 会话8b | **数据预览按钮无功能** | 原来只 `console.log` | ✅ 已修复（2026-08-20 会话8b）：生成 `SELECT * FROM db.table LIMIT 100` 新标签并自动执行 | 接入 `schema:dataPreview` 同样的效果，直接执行 LIMIT 查询 |
| 🆕 会话8b | **查询执行中无法取消** | 工具栏只有「执行」 | ✅ 已修复（2026-08-20 会话8b）：`QueryRequest.clientQueryId` + workspace `executing` 状态，执行中显示「⏹ 停止」调 `query:cancel` | 停止按钮全链路 |
| 🆕 会话8b | **打开/保存用 window.prompt** | `App.tsx` 用 `prompt()` | ✅ 已修复（2026-08-20 会话8b）：`dialog:showOpenDialog`/`showSaveDialog` + `lastScriptDir` 目录记忆 | 原生对话框 |
| 🆕 会话8b | **连接状态无指示** | `ConnectionManager.tsx` 列表无状态 | ✅ 已修复（2026-08-20 会话8b）：`connections:testById` 自动测试 + 绿/灰/红圆点 | 圆点指示 |
| 🆕 会话8b | **对象树无右键菜单** | `ObjectExplorer.tsx` 只有双击 | ✅ 已修复（2026-08-20 会话8b）：右键菜单（数据预览/查看 DDL/刷新） | contextmenu 事件 |
