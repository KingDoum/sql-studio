...
| 会话8 | 体验优化 | 7 项体验优化（会话8） | ✅ completed | 2026-08-20 | 见 §14.1 完整状态 |
| 会话8b | 体验优化遗留项 | 5 项遗留优化（会话8b） | ✅ completed | 2026-08-20 | 预览按钮、停止按钮、原生对话框、连接状态、右键菜单 |
| 会话8c | 体验优化第三轮 | 6 项（会话8c） | ✅ completed | 2026-08-20 | 数据预览弹窗、历史搜索、CSV导出、INSERT表名modal、逗号别名、审查修复 |
| 会话8d | 体验优化第四轮 | 4 项（会话8d） | ✅ completed | 2026-08-20 | CSV backpressure、导出定位文件、收藏重命名、收藏搜索 |

**当前状态：全部 13 + V2 任务已完成 + 会话8 四轮体验优化完成。** 全量验证：tsc 零错误 + vitest 227/227（28 测试文件）+ npm run build 通过。新增 8 个 IPC 通道（settings:get/set、dialog:showSaveDialog、dialog:showOpenDialog、connections:testById、export:csv、favorites:rename、shell:showItemInFolder → 共 35 个）。
...
| ⏳ 待办 | UI | 打开/保存/另存为使用 `window.prompt` | `dialog:showSaveDialog` 已就绪，`dialog:showOpenDialog` 待新增 | `App.tsx` |
...
## §14 Bug 修复与体验优化专项指引（新话题用）

> 目标：针对新话题的 **Bug 修复** 任务。本文档给出已知问题清单、排障方法论、代码入口与验证流程。
> 历史：会话8/8b/8c/8d 已完成 4 轮体验优化（见 §14.1 历史状态，均为 ✅）。

### 14.0 排障方法论（Bug 出现时按此流程）

```
1. 复现：让用户复现操作路径 + 记录错误文本（console/主进程日志/UI 弹窗）
2. 定位：
   ├─ 渲染层问题 → src/renderer/（组件/store/lib）
   ├─ IPC 问题 → src/shared/ipc-contract.ts（通道契约）→ src/main/ipc.ts（handler）
   ├─ 主进程服务 → src/main/services/*.ts
   └─ 构建/打包 → vite*.config.ts / electron-builder.yml / .github/workflows/build.yml
3. 修复：改代码 → tsc --noEmit 零错 → vitest run 全绿 → npm run build
4. 验证：R3 三门槛 + 对应单测补覆盖
5. 发布：git push → CI 构建成功 → 下载 .exe 到 DSH_projects/sql_plus/
6. 记录：架构文档 §14.1 更新状态 + 开发文档 §7 新增坑位
```
...
| 症状 | 最可能根因 | 检查位置 |
|---|---|---|
| `window.sqlStudio` undefined / cannot read properties | preload 构建 external 吞掉 `@shared/*`；或 main/index.ts 未调 registerIpc | `vite.main.config.ts` external 函数 + `src/main/index.ts` |
| `no handler registered for xxx` | `registerIpc()` 未执行或 handler 未注册 | `src/main/index.ts` whenReady → `registerIpc(deps, ipcMain)` |
| 编辑器一直"加载中" | Monaco CDN 被 CSP 拦截 | `main.tsx` 需 `loader.config({ monaco })`，CSP 加 `worker-src blob:` |
| 表单点保存没反应 | jsdom/HTML5 校验拦截 submit | `<form noValidate>` + 自定义校验 |
| 测试连接显示成功但实际失败 | 返回值被丢弃 | `const res = await ...; if (!res.ok) throw` |
| 补全重复/内存泄漏 | provider 每次挂载重复注册 | 模块级 `completionProviderRegistered` 守卫（声明在组件外！） |
| 快捷键不生效 | useEffect 依赖 ref 空跑 | 移入 `onMount` 注册 |
| 展开表/切连接数据串台 | 无请求令牌 | `requestIdRef` 每次请求自增 + 响应校验 |
| 导出没反应 | `window.prompt` 在 Electron 返回 null | 用 `dialog:showSaveDialog` |

### 14.1 已知问题清单（新话题开工先看这里）

> 状态：`✅ 已修复` / `⏳ 待办`。新话题优先处理 ⏳ 项，修复后把本行改为 ✅ 并填日期。

| 优先级 | 问题 | 位置 | 现状 | 修复方向 |
|---|---|---|---|---|
| 🟡 中 | **数据预览（右键/按钮）** | `ObjectExplorer.tsx` + `App.tsx` | ⏳ 待办（会话8b 已在右键菜单加「数据预览」，检查是否端到端可用） | `schema:dataPreview` IPC / 生成 LIMIT 查询新标签 |
| 🟡 中 | **打开脚本原生对话框** | `App.tsx` handleOpen | ⏳ 待办（`dialog:showOpenDialog` 已在 8b 注册，确认接线） | 复用已注册通道 |
| 🟢 低 | **双击表后标签命名** | `App.tsx` handleOpenTable | ⏳ 待办 | 用 `db.table` 作标签标题 |
| 🟢 低 | **收藏重命名 UI** | `FavoritesPanel.tsx` | ⏳ 待办（`favorites:rename` 已在 8d 注册，确认 UI 接线） | 复用已注册通道 |

> ⚠️ 注意：以上 ⏳ 项在会话8b/8c/8d 的代码中大多已注册 IPC 通道或部分实现，**新话题先 `git pull` 最新 master 再核对实际状态**，不要凭本文档旧记录猜测。

### 14.2 历史状态（会话8 四轮体验优化，均已 ✅）

| 会话 | 修复项 |
|---|---|
| 会话8 | 字段注释显示、Excel 导出原生对话框+目录记忆、字段补全表别名、筛选行错位+列宽拖拽、默认库跳过系统库、标签切换卡顿（path 单实例）、双击字段插入 |
| 会话8b | 数据预览按钮、停止按钮全链路、打开/保存原生对话框、连接状态圆点、右键菜单 |
| 会话8c | 数据预览弹窗、历史搜索、CSV 导出、INSERT 表名 modal、逗号别名、审查修复 |
| 会话8d | CSV backpressure、导出定位文件（shell:showItemInFolder）、收藏重命名、收藏搜索 |
| 会话8e | 跨库补全（异步loader）、Excel导出分批写入（setImmediate+大数据量提示）、白天主题（浅色CSS+Monaco light主题+SettingsPanel切换）、Ctrl+S保存、调试模式（环形缓冲日志+一键复制）、AI行内补全与弹窗补全共存 |

### 14.3 Bug 修复作业流程（新话题开工步骤）

1. 读本文档 §14.0 排障方法论 + §14.1 已知问题清单（页顶「当前状态」看最新进度）
2. `git pull` 最新 master（会话8d 后测试基线 227/227，28 测试文件）
3. 复现 → 读目标代码 → 修复 → **保持 227+ 测试全绿**
4. 符合 R3 三门槛（tsc + vitest + build）后推送，CI 验证
5. 更新本文档 §14.1 状态 + 开发文档 §7 坑位

### 14.4 留意点（不要破坏的契约）

- **R5 类型唯一来源**：不复制类型到进程内部；新增 IPC 通道同步 `ipc-contract.ts` + 契约测试硬编码列表
- **R6 渲染零直连**：一切经 IPC，不直连数据库/文件
- **新增组件**：样式追加 `theme.css`（按区域分组），测试文件头加 `// @vitest-environment jsdom`
- **35 个 IPC 通道已注册**：新增前先查 `ipc-contract.ts`，避免重复
- **发布链路**：见开发文档 §13 / 记忆「sql-studio-build-release-workflow」（GH_TOKEN 首次使用会过期，过期找用户要新 token）