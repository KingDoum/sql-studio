...
| 分类 | 限制 | 期望改进 | 涉及文件 |
|---|---|---|---|
| UI | 打开/保存/另存为使用 `window.prompt` 输入路径 | 改为 Electron 原生文件对话框 | `App.tsx` handleOpen/handleSave |
| 构建 | @monaco-editor/react 默认从 CDN 加载 monaco | 配置 `loader.config({ monaco })` 走本地 | `SqlEditor.tsx` beforeMount |
| 功能 | 查询取消按钮未接入 UI | 执行中显示「停止」按钮，调用 `query:cancel` | `App.tsx` + `SqlEditor.tsx` |
| 功能 | 数据预览（右键表 → 预览数据）未实现 | `schema:dataPreview` IPC 已注册，需 UI 接入 | `ObjectExplorer.tsx` |
| 功能 | 重命名收藏未实现 | FavoritesPanel 添加重命名功能 | `favorites-store.ts` + `FavoritesPanel.tsx` |
| 测试 | GUI 冒烟需要图形环境 | 在 Windows 开发机执行 `npm run dev` 补验 | — |
| CI | Windows 构建约 10-15 分钟 | 可配置缓存加速 | `.github/workflows/build.yml` |